...
class DashboardRepository
{
    ...
    {
        $meetings = $this->meeting->myMeeting()->where('start_date_time', '>', now())->isScheduled()->orderBy('start_date_time', 'asc')->take(5)->get();

        $recent_meetings = $this->meeting->myMeeting()->where('meta->ended_at', '<', now())->isNotCancelled()->orderBy('meta->ended_at', 'desc')->take(5)->get();
