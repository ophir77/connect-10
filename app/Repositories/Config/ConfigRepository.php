...
class ConfigRepository
{
    ...
    {
        return \Cache::remember('query_list_all_config', 60, function () {
    ...
    public function getConfig() : array
    {
        ...
        $ice_configs = $this->getIceConfig($db_config);
    ...
    private function getPublicConfig($db_config = array(), $authenticated = false, $two_factor = false) : array
    {
        ...
        foreach ($public_config as $key => $config) {
            foreach ($config as $item) {
                $configs[$key][$item] = Arr::get($db_config, $key.'.'.$item) ? : Arr::get($default_config, $key.'.'.$item);
            }
        }
    ...
    private function getIceConfig($db_config) : array
    {
        ...
        foreach ($servers as $server) {
            ...
            if (Arr::get($server, 'requires_credential')) {
                ...
                $ice_servers[] = array('urls' => Arr::get($server, 'urls'));
            }
        }
    ...
    private function addGeneralConfig($app = array()) : array
    {
        ...
        $app['system']['currency'] = ArrHelper::searchByKey(ArrHelper::getVar('currencies'), 'name', Arr::get($app, 'system.currency'));
    ...
    private function initSystemDefault() : void
    {
        ...
        date_default_timezone_set(config('config.system.timezone') ?: 'Asia/Kolkata');
        \App::setLocale(config('config.system.locale') ?: 'en');
    }
    ...
    private function setAuthConfig()
    {
        request()->validate([
            ...
            'lock_screen_timeout'           => 'required_if:lock_screen,1|integer|min:1|max:10',
    ...
    private function setPusherConfig() : void
    {
        request()->validate([
            ...
            'pusher_app_id'      => __('config.pusher.app_id'),
            'pusher_app_key'     => __('config.pusher.app_key'),
            'pusher_app_secret'  => __('config.pusher.app_secret'),
            'pusher_app_cluster' => __('config.pusher.app_cluster')
        ]);
