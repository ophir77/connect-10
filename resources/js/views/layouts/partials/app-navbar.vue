<template>
    <nav class="navbar navbar-design-basic navbar-expand-sm d-flex justify-content-around align-items-center flex-nowrap">
        ...
        <a class="nav-link toggle-left-sidebar d-sm-none pr-4" @click="toggleLeftSidebar"><i class="fas fa-bars"></i></a>

        <app-logo place="navbar" />

        <ul class="nav nav-inline d-none d-lg-flex mr-2">
            <li class="nav-item">
                <a class="nav-link toggle-left-sidebar" v-b-tooltip.hover :title="$t('layout.toggle_sidebar')" @click="toggleLeftSidebar"><i class="fas fa-bars"></i></a>
            </li>
            <li class="nav-item">
                <a class="nav-link toggle-full-screen" v-if="uiConfigs.fullScreen" v-b-tooltip.hover :title="$t('layout.toggle_full_screen')" @click="toggleFullScreen(false)"><i class="fas fa-compress"></i></a>
                <a class="nav-link toggle-full-screen" v-else v-b-tooltip.hover :title="$t('layout.toggle_full_screen')" @click="toggleFullScreen(true)"><i class="fas fa-expand"></i></a>
            </li>
        </ul>
        ...
        <ul class="nav nav-inline d-none d-sm-flex">
            <!-- <DropdownTasks /> -->
        ...
        <ul class="navbar-nav">
            ...
            <base-dropdown tag="li" class="nav-item d-flex" position="right" :open.sync="userDropddownOpen">
                <a slot="title" class="nav-link nav-link-icon nav-user-avatar p-0" data-toggle="dropdown" role="button">
                    ...
                    <avatar v-if="loggedInUser && loggedInUser.name" :username="loggedInUser.name" :src="loggedInUser.avatar" :background-color="vars.colors.white" :color="vars.colors.primary" :size="40"></avatar>
                    <avatar v-else username="User" :background-color="vars.colors.white" :color="vars.colors[uiConfigs.topNavbarColor]" :size="40"></avatar>
                </a>
                ...
                <router-link v-if="loggedInUser" :to="{ name: 'appProfileView' }" class="dropdown-item">
                    <i class="fas fa-user"></i> {{ $t('user.profile') }}
                </router-link>
