...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // Call related
            incomingCallNotification({ from, to, id }) {
                if(to.uuid !== this.uuid) {
                    return 
                }

                window.callPlaying = playIncomingCall()

                const alertContent = {
                    title: `${from.name} is calling ...`,
                ...
                this.$toasted.clear()
                this.setUserStatus('GETTING_CALLED')
                window.callFrom = from

                callAlert.fire(alertContent)
                    .then((result) => {

                        window.callPlaying.pause()
                        window.callFrom = null
                        
                        if (result.value) {
                            this.setUserStatus('ON_CALL')

                            this.subscriptions.users.whisper('CallAccepted', { 
                                to: from,
                                from: to,
                                id: id,
                            })
                            ...
                            if(result.dismiss === Swal.DismissReason.cancel) {
                                ...
                                this.subscriptions.users.whisper('CallRejected', { 
                                    to: from,
                                    from: to,
                                    id: id,
                                })

                                this.$toasted.error(`Call from ${from.name} rejected`, this.$toastConfig.error)

                            } else if(result.dismiss === Swal.DismissReason.timer) {

                                this.$toasted.info(`Call missed from ${from.name}`, this.$toastConfig.info)

                            }
                        }
                    })
            },
            openCallWindow({ from, to, id }) {
                const callWindow = window.open(`/app/live/calls/${id}?from=${from.uuid}&to=${to.uuid}`, '_blank')

                popupBlockerChecker.check(callWindow)
                ...
                if(callWindow && !callWindow.closed && typeof callWindow.closed!='undefined') 
                {
                    ...
                    this.subscriptions.users.whisper('CallEnded', { 
                                to: to,
                                from: from,
                                id: id,
                            })

                            this.$toasted.info(`Call ended!`, this.$toastConfig.info)
                        }
                    }, 1000)
                }
            },
            callAcceptedCallback({ from, to, id }) {
                if(to.uuid !== this.uuid) {
                    return 
                }
                ...
                // this.$toasted.success(`Call accepted from ${from.name}`, this.$toastConfig.success)
            ...
            callCancelledNotification({ from, to, id }) {
                if(to.uuid !== this.uuid) {
                    return 
                }
            ...
            callRejectedNotification({ from, to, id }) {
                if(to.uuid !== this.uuid) {
                    return 
                }
                ...
                this.$toasted.error(`Call rejected from ${from.name}`, this.$toastConfig.error)
            },
            callDroppedNotification({ from, to }) {
                window.callPlaying.pause()
                Swal.close()
                this.setUserStatus(false)

                if(from.uuid === this.uuid) {
                    window.callTo = null
                    window.callUuid = null

                    this.$toasted.error(`Error calling ${from.name}`, this.$toastConfig.error)
                } else if(to.uuid === this.uuid) {
                    window.callFrom = null
                    window.callUuid = null

                    this.$toasted.error(`Error while connecting to ${from.name}`, this.$toastConfig.error)
                }
            },
            beforeUnload(event) {
                if(window.callFrom) {
                    this.subscriptions.users.whisper('CallDropped', { 
                        from: window.callFrom,
                    })
                }
            },
        ...
        mounted() {
            ...
            if(window.Echo && window.EchoOpts) {
                // window.EchoOpts.subscriptions.privateUser = window.Echo.private(`Users.${this.uuid}`)
                window.EchoOpts.subscriptions.users = window.Echo.join(`Users`)
                this.subscriptions = {
                    // privateUser: window.EchoOpts.subscriptions.privateUser,
                ...
                this.subscriptions.users.here(this.afterJoiningUsersChannel)
                    ...
                    .listenForWhisper('StartCalling', this.incomingCallNotification)
                    .listenForWhisper('CallCancelled', this.callCancelledNotification)
                    .listenForWhisper('CallRejected', this.callRejectedNotification)
                    .listenForWhisper('CallDropped', this.callDroppedNotification)
                    .listenForWhisper('CallAccepted', this.callAcceptedCallback)
            }

            // this.subscriptions.privateUser.notification(this.socketSubscriptionCallback)
        },
        created() {
        ...
        beforeDestroy() {
            ...
            if(window.Echo) {
                if(window.EchoOpts) {
                    // window.EchoOpts.subscriptions.privateUser.stopListening('.Illuminate\\Notifications\\Events\\BroadcastNotificationCreated', this.socketSubscriptionCallback)
                }
                this.subscriptions = {}
                // window.Echo.leave(`User.${this.uuid}`)
